...
export default function OnboardingModal() {
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      ...
      setSuggestions(
        data.suggestions.map((s: { name: string; description: string; examples: string }) => ({
          ...s,
          enabled: true,
        }))
      );
  ...
  const handleAccept = async () => {
    ...
    setStep("applying");
    setApplyStatus("Removing default buckets...");

    try {
      // Delete default buckets (that have 0 threads)
      const bucketsRes = await fetch("/api/buckets");
      const buckets = await bucketsRes.json();
      for (const b of buckets) {
        if (b.isDefault && b._count.threads === 0) {
          await fetch(`/api/buckets/${b.id}`, { method: "DELETE" });
        }
      }

      // Create new buckets
      setApplyStatus("Creating your buckets...");
  ...
  const updateBucket = (idx: number, field: keyof SuggestedBucket, value: string) => {
    setSuggestions((prev) =>
        ...
        <div className="px-6 pt-6 pb-4 border-b border-stone-100 flex-shrink-0">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-stone-500">AI is analyzing your emails to suggest buckets</p>
            </div>
          </div>
        </div>
          ...
          {error && (
            <div className="py-8 text-center space-y-4">
              ...
              <div className="flex gap-2 justify-center">
                <button
                  onClick={() => { setError(null); fetchSuggestions(); }}
          ...
          {step === "review" && (
            <div className="space-y-2">
              <p className="text-xs text-stone-400 mb-3">
                Toggle, edit, or remove buckets before applying. These replace the defaults.
              </p>
              {suggestions.map((s, i) => (
                <div
                  key={i}
